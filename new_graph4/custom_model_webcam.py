...
PATH_TO_TEST_IMAGES_DIR = 'outputs2'
TEST_IMAGE_PATHS = [os.path.join(PATH_TO_TEST_IMAGES_DIR, 'detection_output{}.png'.format(i)) for i in range(0, 100)]  # adjust range for # of images in folder
PATH_TO_SHOW_IMAGE_DIR = 'output'
...
with detection_graph.as_default():
    with tf.Session(graph=detection_graph) as sess:
        ...
        for image_path in TEST_IMAGE_PATHS:
            ...
            cv2.imwrite("outputs2/detection_output{}.png".format(i), preimg)
            print("ok-")
            ...
            print ("run ")
            imgout = cv2.imread("outputs/detection_output{}.png".format(i))
            
            cv2.imshow("output", imgout)
            i = i+1
            #cv2.resize(IMAGE_SIZE)
            #cv2.imshow(image_np)
            cv2.waitKey(1)
            #time.sleep(0.5)
